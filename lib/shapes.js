...
class Circle extends Shapes {
    ...
    render(colorS, colorT) {
        if(colorS === 'green' && colorT === 'white') {
            const trial = `<circle cx="150" cy="100" r="100"  stroke="${colorS}"  fill="${colorS}" stroke-width="2"/><text x="135" y="100" fill="${colorT}">Text</text>`
            return trial
        } 
        return `
...
class Square extends Shapes {
    ...
    render(colorS,colorT) {
         if(colorS === 'green' && colorT === 'white') {
            const trial = `<rect x="50" y="0" width="200" height="200" stroke="${colorS}" fill="${colorS}" stroke-width="2"/><text x="135" y="100" fill="${colorT}">Text</text>`
            return trial
        } 
...
class Triangle extends Shapes {
    ...
    render(colorS, colorT){
        if(colorS === 'green' && colorT === 'white') {
            const trial = `<polygon  points="103,0 200,150 15,150" fill="${colorS}"  stroke="${colorS}"  stroke-width="1"/><text x="85" y="100" fill="${colorT}">Text</text>`
            return trial
        } 
